...
namespace ttc
{
    ...
    TensorTradeObserver::TensorTradeObserver(Portfolio *portfolio, DataFeed *dataFeed,
        ...
        auto internal_group = group(move(streams));
        internal_group->rename<Stream>("internal");

        auto external_group = group(dataFeed->Inputs());
        external_group->rename<Stream>("external");
        ...
        {
            feed = std::make_unique<DataFeed>(std::vector<Float64Stream>{internal_group, external_group});
        }

        this->window_size = window_size;
        this->min_periods = min_periods;

        auto initial_obs = feed->next<unordered_map<string, unordered_map<string, double>>>().at("external");
            ...
            while (i++ < min_periods.value())
            {
                if(hasNext())
                {
                    history.push(tensorView(feed->next<unordered_map<string,
                                            unordered_map<string, double>>>().at("external"))); // view of values
                }
            }
        }
    }
    ...
    torch::Tensor TensorTradeObserver::observe(struct TradingEnv *env) {
        ...
        auto data = feed->next<unordered_map<string, unordered_map<string, double>>>();
